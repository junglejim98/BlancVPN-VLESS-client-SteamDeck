...
func getConnectionStatus() RuntimeStatus {
	...
	xrayRunning := isProcessAliveFromPIDFile(xrayPIDPath)
	tun2socksRunning := isProcessAliveFromPIDFile(tun2socksPIDPath)
	tunReady := isTunInterfaceReady(runtimeTunName)

	if xrayRunning && tun2socksRunning && tunReady {
		return RuntimeStatusConnected
	}

	if xrayRunning || tun2socksRunning {
		return RuntimeStatusConnecting
	}
